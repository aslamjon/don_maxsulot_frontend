...
const showError = (e, setError, isField = true) => {
    ...
    //     }
    // });
    if (e?.message) toast.error(get(e, "message"));
};
...
const getSelectOptionsListFromData = (data = [], value = 'id', label = 'title', other) => {
    if (other) return data.map(item => ({ ...item, value: item[value], label: item[label] })) || [];
...
function rgbaTohex(orig) {
    if (orig.startsWith("rgba")) {
        ...
        if (numbers.length !== 4) return "error length";
            ...
            let red = Number(numbers[0]), green = Number(numbers[1]), blue = Number(numbers[2]),
...
const search = (data = [], str = '', field = 'name') => {
    return data.filter((item) => includes(toLower(get(item, `${field}`)), toLower(str)) || includes(toLower(str), toLower(get(item, `${field}`))));
}
