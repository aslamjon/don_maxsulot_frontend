...
const StyledInput = styled.div`
  .form-input-container {
    ...
    ${({ isOpen, isFocused, magic, typee }) => typee === "password" && !isOpen && css`
        &:after {
            content: "${magic}";
            position: absolute;
            top: 1px;
...
const Input = ({
    ...
    useEffect(() => {
        if ((isEmpty(getValues(name)) || (getValues(name) !== defaultValue)) && isString(defaultValue)) {
            setValue(name, defaultValue);
            onChange(defaultValue);
        }
    }, [defaultValue]);
    ...
    useEffect(() => {
        if (defaultColor === undefined || state.color === undefined || state.color !== defaultColor) setState(s => ({ ...s, color: defaultColor }));
    }, [defaultColor]);
    ...
    const handleChangeForColor = (e) => {
        ...
        if (addColor && addColorInValue && colorKey) setValue(name, { [inputKey]: get(getValues(name), inputKey, ""), [colorKey]: e });
    }

    const getValueForInput = () => {
        let temp = "";
        if (addColor && addColorInValue && colorKey) temp = get(getValues(name), inputKey, "");
        ...
        <StyledInput {...{
            ...
            <Row className={rowClassName}>
                ...
                <Col className={`${colClassName}`} xs={last(cols)}>
                    <div className={`form-input-container ${state.isFocused && "focused"}`} onDoubleClick={(e) => get(property, 'onDoubleClick', () => { })(e)}>
                        {!(get(property, "type") === 'money') ? <input
                        ...
                        {/*{addColor && state.showColor && <OutsideClickHandler onOutsideClick={() => setState(s => ({ ...s, showColor: false }))}>*/}
                        {/*    <ColorPicker className={"color-picker"} myColor={state.color} handleChange={handleChangeForColor} />*/}
                        {/*</OutsideClickHandler>}*/}
                        {addColor && <div className="colorLabel" onClick={() => setState(s => ({ ...s, showColor: !s.showColor }))}>
                            {colorLabel}
                        </div>}
                    </div>
                    {!hideError && isError() &&
