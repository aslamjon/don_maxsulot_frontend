                ...
                cb: {
                    success: (data) => {
                        setGroupedViewData(groupedViewData => ({
                            ...groupedViewData,
                            [statusId]: [...get(groupedViewData, `${statusId}`, []),
                                ...get(data, 'data', [])
                            ]
                        }));
                        setScrolling(true);
        ...
        <Style {...rest}>
            <div>
                <div className="board-header">
                    {
                        isArray(statusList) && statusList.map((status, index) => <div className="head-and-card"
                                                                                      key={index}>
                            <BoardColumn
                                ...
                                count={get(find(idList, item => isEqual(get(item, 'statusId'), get(status, 'id'))), 'count', 0)}
                                scrolling={scrolling}
                            />
                        </div>)
                    }
