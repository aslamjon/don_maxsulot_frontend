        ...
        <div className="list__body">
            {body.map((val, ind) => (
                <div className="list__body__row" key={val.id + 35}>
                    <div className="list__body__row__border"><BorderStyled color={statusColor} /></div>
                    <div className="list__body__row__name"> {val.name} </div>
                    <div className="list__body__row__content">
                        {title && isArray(title) && title.map(([{ content }], indem) => (
                            content == "priority"
                                ...
                                : content == "call"
                                    ? <div className={`list__body__row__${content}`} key={indem}>
                                        {val.call && <span><Icon icon="icon-phone" color="white" /></span>}

                                    </div>
                                    : content == "message"
                                        ? <div className={`list__body__row__${content}`} key={indem}>
                                            {val.message && <span><Icon icon={'icon-message'} color="white" /></span>}
                                        </div>
                                        : content == "action"
                                            ? <div className={`list__body__row__${content}`} key={indem}>
                                                {/* {val.action.phone && <span><Icon icon="icon-phone" color="white" /></span>}
                                        {val.action.mail && <span><Icon icon={'icon-message'} /></span>} */}
                                                {<span><Dropdown button={<Icon icon="icon-more-dots" color="dark" />}>test</Dropdown></span>}
                                            </div>
                                            : <div className={`list__body__row__${content}`} key={indem}>{val[content]}</div>
                        ))}
                    </div>
                </div>
            ))}
        ...
        <div className="list">
            <div className="list__head">
                <div className="list__head__status">
                    <Button className={`list__head__status__toggle ${isOpen && "active"} ${head.statusColor == "#FFD166" && "black"}`}
                        bg={head.statusColor}
                        ...
                        {head.status}
                        <Icon icon="icon-bottom-arrow" listClassName="bottomArrow" color={head.statusColor == "#FFD166" ? "#353945" : 'white'} />
                    </Button>
                    <span>{body.length} LEAD</span>
                    {isOpen && <Icon icon="icon-shape" />}
                    {isOpen && <Icon icon="icon-add-plus" size="xs" />}
                </div>
                {isOpen && <div className="list__head__dragDrop">
