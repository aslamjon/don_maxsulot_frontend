...
const ModalBody = ({ addOrEdit, cancel, item, btnText = 'Add', limitFile, openModal }) => {
    ...
    let defaultValue = (isEmpty(item) && openModal) ? head(get(item, "optionDTO.values", 0)) : head(get(item, "optionDTO.values", " "));
    return (
        <FormDemo  {...temp} formRequest={(data) => {
            ...
            <Row>
                ...
                <Col xs={12} className={'mb-20'}>
                    <Field type={'input'} defaultValue={`${get(item, 'maxMb')}`} name={'maxMb'} label={'TYPE limit file size'} params={{ required: true }} />
                </Col>
                <Col xs={12}>
                    <Flex justify={'flex-end'} align={'center'}>
                        <Button className="cancelBtn" outlineDanger onCLick={cancel}>Cancel</Button>
                        <Button className="addBtn" success type={'submit'}>{btnText}</Button>
                    </Flex>
                </Col>
            </Row>
        </FormDemo>
    ...
    return {
        getFormData: () => {
            dispatch({
                ...
                payload: { url: 'attachment/v1/limit-file-size/get/1', method: 'get', storeName: 'limit-file-size-form-data' }
            })
        },
    }
}
