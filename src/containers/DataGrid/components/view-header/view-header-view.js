...
const ViewHeaderViews = ({ t, viewTypesList = {}, redirectUrl, createView, deleteView, getView, viewTypesListState, setSelectedViewType, selectedViewType }) => {
    return (
        <Style>
            <Dropdown
                className="dropDown__filter view_drop"
                button={<Button className="plus" outline_success> {t("View") ?? "View"}</Button>}>
                        ...
                        <Col xs={9} className="view__left">
                            <div className={'view__left__content'}>
                                {get(getView(selectedViewType), `defaultView.defaultView`, false) &&
                                    <ViewRowReport
                                        redirectUrl={get(redirectUrl, 'view')}
                                        {...get(getView(selectedViewType), `defaultView`, {})}/>}
                                {
                                    get(getView(selectedViewType), `views`, []).map((view) =>
                                        <ViewRowReport
                                            deleteView={deleteView}
                                            redirectUrl={get(redirectUrl, 'view')}
                                            key={get(view, 'id')} {...view}/>)
                                }
                            </div>

                            {get(viewTypesListState, 'result.data.permissions.canCreateView', false) &&
                                <div className="view__left__form">
                                    <Field sm className="personalPublic" type="checkbox" name="personal"
                                           label={t("Personal view") ?? "Personal view"}/>
                                    <Button className="addNewList" type={'submit'}
                                            success>{t("Add new list") ?? "Add new list"}</Button>
                                </div>}
                        </Col>
                            ...
                            <div className="view__right__container">
                                <div className="view__right__container__top">
                                    <Label className="view__right__container__top__label">
                                        ...
                                        <Field className="view__right__container__top__label__input"
                                               property={{placeholder: (t("Enter name...") ?? "Enter name...")}}
                                ...
                                <div className="view__right__container__body">
                                    ...
                                    {get(viewTypesList, 'result.data.viewTypes', []).map((type) => ({
                                        icon:
                                            <>
                                                <Icon
                                                    icon={`icon-${lowerCase(get(type, 'name'))}`}/> {get(type, 'name')}
                                            </>,
                                        type: get(type, 'name')
                                    })).map(({icon, type}, ind) =>
                                        <Button key={ind + 1}
                                                className={classNames(`view__right__container__body__button`, {selected: isEqual(selectedViewType, type)})}
                                                onClick={() => setSelectedViewType(type)}> {icon} </Button>)}
                                </div>
                            </div>
                        </Col>
                    </Row>
