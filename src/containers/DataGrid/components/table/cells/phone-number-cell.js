...
const CustomHeader = ({
    ...
    return <>
        ...
        {<CustomIcon {...{isMulti, selected, isClearAll, clearAll, clickHeader}} />}
    </>
}
const CustomOption = ({
                          options,
    ...
    useEffect(() => {
        if (!isEmpty(get(typeConfig, "phoneNumberConfig.phoneOptions", []))) {
            ...
            if (isNull(initialValue) ? "" : initialValue) {
                ...
                if (!isEmpty(res))
                    selected = get(typeConfig, `phoneNumberConfig.phoneOptions[${get(res, "index",0)}]`, "");
            }
            setData(s => ({...s, selected, value: isNull(initialValue) ? "" : initialValue.replace(selected.callingCode, "") }));
        }
    }, [typeConfig]);
    ...
    const handleEnter = (event) => {
        if (event.keyCode === 13) {
            if (data.value.length > 7 && data.value.length < 18) {
                setData(s => ({...s, valid: true, focusColumn: false, initialValue: `${get(data, "selected.callingCode", "")}${data.value}` }))
                setLoading(true);
                updateItemRequest({
                    id: rowId,
                    attributes: {[id]: `${get(data, "selected.callingCode", "")}${data.value}`},
                    cb: {
                        success: () => {
                            setLoading(false);
