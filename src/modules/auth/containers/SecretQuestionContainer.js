...
const SecretQuestionContainer = ({match, history, request, t, password, checkAuth, saveToken }) => {
    ...
    const goToBack = () => {
        if (options.length === Number(page))
            history.push(`/auth/verification/${get(match, "params.phone", {})}/${btoa("123")}/${btoa("123")}/${get(match, "params.options")}?verificationType=${btoa("123")}`);
        else if (page === "1")
            history.push(`/auth/verification-methods/${get(match, "params.phone", "")}/${get(match, "params.options", "")}/${get(match, "params.type", "")}`);
        else if (page === "2") {
            let questions = btoa(btoa(JSON.stringify(question)));
            history.push(`/auth/secret-question/${get(match, "params.phone", {})}/${get(match, "params.options", {})}/${questions}/${btoa("1")}/${get(match, "params.type", "")}`);
        }

    }

    console.log(atob(get(match, "params.type", "")));
    const submitHandling = useCallback(({data: {answer}}) => {
            ...
            cb: {
                success: ({data: {questionId, questionText, accessToken, ...other}}) => {
                    ...
                    if (options.length === Number(page)) {
                        ...
                        let questions = {...question, [Number(page) + 1]: {questionId, questionText}};
                        questions[page] = {...questions[page], answer};
                        questions = btoa(btoa(JSON.stringify(questions)));
                        history.push(`/auth/secret-question/${get(match, "params.phone", {})}/${get(match, "params.options", {})}/${questions}/${btoa(String(Number(page) + 1))}/${get(match, "params.type", "")}`);
                    }
                },
                fail: (e) => {
                    showError(e);
    ...
    return (
        <StyledSecretQuestion>
            <Title className="title" medium fs={32} lHeight={48}>{t("secret_question") ?? "Secret Question"}</Title>
            <Title
                medium
                lHeight={30}
                fs={20}>
            ...
            <FormDemo {...state.temp} formRequest={submitHandling}>
                <Field
                    ...
                    autoFocus
                    onChange={(value) => (value && state.disable)
                ...
                <Flex justify={"space-between"}>
                    ...
                    <Button
                        ...
                        className='nextButton'>{state.loading ? <MiniLoader/> : t("next") ?? "Next"}</Button>
                </Flex>
