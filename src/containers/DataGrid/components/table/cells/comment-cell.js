        ...
        <Style>
            <Dropdown button={isNull(initialValue) ? 0 : initialValue.length} clickButton={handling}>
                <SimpleBar className="barBody">
                    ...
                    {isArray(state) && state.map(item => (
                        <div className={"box"}>
                            <Img className={"box__img"}
                                 src={!isNull(get(item, "photoId", null)) ? `${get(item, "avatar", '')}?id=${get(item, "photoId", null)}&view=open&format=SMALL` : useImg}/>
                            <div className="box__container">
                                <div className="box__head">
                                    <div className="box__head__fullName">
                                        {get(item, "fullName", "")}
                                    </div>
                                    <div>
                                        {formatDate(new Date(get(item, "createdAt", 0)), "dd/MM/yyyy")}
                                    </div>
                                </div>
                                {get(item, "text", "")}
                            </div>
