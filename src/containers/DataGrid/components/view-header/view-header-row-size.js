...
`;
const ViewHeaderRowSize = ({
                               t,
                               changeRowSize = () => {
                               },
                               defaultRowSize = 'SMALL',
                               ...rest
                           }) => {
    ...
    return (
        <Style {...rest}>
            <Dropdown
                className={"resizer"}
                button={<img src={ViewIcon}/>}>
                {isArray(rowSizes) && rowSizes.map(({value, label}, index) => <div
                    onClick={() => {
                        setRowSize(value);
                        changeRowSize(value)
                    }}
                    key={index + 1}
                    className={classNames('options', {check: isEqual(rowSize, value)})}>{label}</div>)}
            </Dropdown>
