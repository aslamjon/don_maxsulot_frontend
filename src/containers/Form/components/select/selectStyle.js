...
export const colourStyles = {
    control: (styles, {isDisabled, isFocused, isSelected, selectProps: {theme: {mode}, checked, error}}) => {
        return ({
            ...styles,
    ...
    menu: (styles, {selectProps: {theme: {mode}}}) => ({
        ...styles,
        transition: "0.3s",
    ...
    placeholder: (styles, { selectProps: { theme: { mode } } }) => ({
        ...styles,
        fontWeight: "normal",
        fontSize: "12px",
        lineHeight: "12px"
    ...
    multiValue: (styles, { data: { color }, selectProps: { theme: { mode } } }) => {
        let rgb = '';
        if (color) {
            rgb = color.split("(")[0];
