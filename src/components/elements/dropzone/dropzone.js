...
const CustomInput = ({ accept, onFiles, files, getFilesFromEvent }) => {
	return (
		<label className="main__upload__label">
			...
			<p >Drag and drop files here or <a> Browse</a></p>
		</label>
	)
}
const DropZone = ({
	...
	const handleChangeStatus = (files, status) => {
		if (status == "preparing") {
			...
			fileUpload({ attributes: formData, formMethods: {setLoading: () => {}}, cb: {
				success: ({ id , url}) => {
		...
		return (
			<DropzoneStyled>
				...
				<div className={classNames({ 'collapse': collapse }, { 'upload__row__wrapper': files.length > 0 })}>
					{files.length > 0 && <div className="dzu-dropzone__upload__counter">
						<div>
							<Text xlg medium>Uploaded {files.length} files</Text>
							{collapse && <div>on collaps progress</div>}
						</div>
						<div>
							{collapse ? <Icon onClick={() => isCollapse(!collapse)} className='icon__exit__full_screen' icon='icon-magnifier' color='#777E91' />
								: <Icon onClick={() => isCollapse(!collapse)} className='icon__exit__full_screen' icon='icon-exit-full-screen' color='#777E91' />
							}
						</div>
					</div >}
								...
								{percent == 100 ?
									...
									<div className="onprogress__row">
										<div className="onprogress__row__option__wrapper">
											...
											<div className="onprogress" style={{ display: 'flex', height: '25px', alignItems: 'center' }}>
												<Text className='onprogress__date__text' medium>{calculateTime()}  left · {Math.round(percent)} %</Text>
												<Icon
													icon="icon-exit"
										...
										<div><progress id="file" value={percent} max="100">  dfs </progress></div>
									</div>
