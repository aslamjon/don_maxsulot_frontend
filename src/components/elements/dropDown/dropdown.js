...
const DropdownStyle = styled.div`
  ...
  &.dropDown {
    &.active {
      .rs-anim-bounce-in, .rs-anim-bounce-out {
        z-index: 2;
      }
      .dropDown__body {
        /* min-width: 140px; */
...
const Dropdown = ({
  ...
  const handleToggle = (e) => {
      ...
      onChange(!show);
      setShow(s => !s);
      isFixed && setLastPosition(s => ({ x: get(e, "clientX",0), y: get(e, "clientY",0)}));
      isFixed && getXandY({ x: get(e, "clientX",0), y: get(e, "clientY",0), other: e });
      clickButton();
  };
  ...
  useEffect(() => {
    ...
    <DropdownStyle
        ...
        <OutsideClickHandler onOutsideClick={()=> {
            ...
            <div className="dropDown__button" onClick={handleToggle}> {button} </div>
            <motion.div animate={{
                x: 0,
                scale: show ? 1 : 0,
                opacity: show ? 1 : 0,
