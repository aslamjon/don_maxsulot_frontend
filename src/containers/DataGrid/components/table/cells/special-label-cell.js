...
const SpecialLabelCell = ({
    ...
    useEffect(() => {
        return () => {
            if (!get(getTempData(), rowId) && !isEmpty(data)) {
                setItem({item: {...getTempData(), ...data}, storeName: "temp-Data-For-Lead-View"});
            }
        }
    }, [data]);
    ...
    const onChangeHandling = (e) => {
        if (e && !get(getTempData(), rowId)) {
            isEmpty(data[rowId]) && getDataById({
                url: `${get(typeConfig, "specialLabelConfig.url", "")}?${getParamsKey()}=${rowId}`,
                storeName: rowId,
                cb: {
                    success: (res) => {
                        setData(s => ({...s, [rowId]: get(res, "data", [])}))
                    }
                }
            })
        }
    ...
    const getData = (key, item) => {
        if (get(config, "time", false) && (get(config, "time", false) === key)) return formatDate(new Date(get(item, key, "")),"dd/MM/yyyy  HH:mm:ss");
                ...
                <SimpleBar style={{ maxHeight: "300px"}} forceVisible={"y"}>
                    ...
                    <div className={"cards"}>
                        {isArray(arr) && arr.map((item, index) => (
                            <div className={"tr"} key={get(item, "id", "")} style={{ background: get(item, config["coloredRow"], false) ? "rgb(218, 232, 245, 0.5)" : "none"}}>
                                { Object.keys(item).map((i, ind) => ignoreRow(i) && (
