...
const FileCell = ({
    ...
    const clickFile = (item, type) => {
        ...
        if (isString(item) && (type === "img")) {
            let ModalBody = ({type, value}) => (
                (type === "url") && <img src={value} alt={value} className={"img"}/>
            );
            ...
            setTemp({
                item: {
                    active: true,
                    Children: ModalBody,
                    props: {value: item.startsWith("blob:http") ? item : url, type: "url"},
                    deleteItem: () => {
                        setLoading(true);
                        updateItemRequest({
                            id: rowId,
                            viewId,
                            attributes: {[id]: isEmpty(values) ? null : values}, cb: {
                                success: () => {
                                    setLoading(false);
            ...
            let ModalBody = ({type, value}) => (
                ...
                <object data={`${value}&view=open`} type="application/pdf" width="100%" height="720px">
                    <p>Alternative text - include a link <a href={`${value}&view=open`}>to the PDF!</a></p>
                </object>
            );
            //    height: 710px;
            setTemp({
    ...
    const onChange = (e) => {
        ...
        if ((file.type === "image/jpeg") || (file.type === "image/png") || (file.type === "image/svg+xml")) temp = URL.createObjectURL(file);
        else temp = file;
    ...
    return (
        <Styled {...{valid: data.valid, ...rest}}>
            {isArray(data.value) && data.value.map((item, index) => (
            ...
            <Dropdown isClose={!data.focus} button={
