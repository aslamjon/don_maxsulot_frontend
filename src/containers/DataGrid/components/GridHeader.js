...
const GridHeader = ({
  ...
  useEffect(() => {
    if (!isEmpty(viewTypesList)) {
      setViewTypesListState(viewTypesList);
    }
  }, [viewTypesList]);
  ...
  const columns = useMemo(
    () => get(view, "columns", []).filter(({ searchable }) => searchable),
    [view]
  );
  ...
  const createView = ({ data, setError }) => {
    addViewRequest({
      attributes: {
        ...
        defaultViewId: get(
          viewTypesListState,
          `result.data.viewTypes[${selectedViewType}].defaultView.id`,
          null
        ),
      },
          ...
          const result = {
            ...
            result: {
              ...
              data: {
                ...
                viewTypes: get(viewTypesListState, "result.data.viewTypes", []).map(
                  (viewTypeItem) =>
                    isEqual(get(viewTypeItem, "type"), get(data, "type"))
                      ? {
                          ...viewTypeItem,
                          views: [...get(viewTypeItem, "views", []), get(data, "data", {})],
                        }
                      : viewTypeItem
                ),
              },
  ...
  const getView = (type = "TABLE") => {
    return find(get(viewTypesListState, "result.data.viewTypes", []), (viewItem) =>
      isEqual(get(viewItem, "name"), type)
    );
  };
  ...
  return (
    <Style>
      <ViewHeaderBox>
        {customLeftElement}
        <div style={{ flex: 1 }}>
          <ViewHeaderSearch searchFields={columns} searchFromView={searchFromView} />
        </div>
        <ViewHeaderViews
          {...{
            t,
        ...
        <ViewHeaderRowSize
          ...
          defaultRowSize={get(view, "rowSize", "SMALL")}
        />
...
export default withTranslation("pdp")(memo(GridHeader));
