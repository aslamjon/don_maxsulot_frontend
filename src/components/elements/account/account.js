...
const Account = ({
    ...
    return (
        <StyledAccount {...props} ref={ref} active={active} userImg={get(user, 'photo', '')} >
            ...
            <Dropdown button={
                get(user, 'photo', '') ?
                    <div className='user_Img' onClick={() => {setActive(state => !state)}} />
                        :
                    <Icon icon={'icon-userImg'} className={'unknown_Img'}  />

            }>
                <div className='dropdawn'>
                    <Button className='drop_btn' onCLick={() => history.push("/account")}><Icon icon="icon-settings" color="#777E90"/>User Settings</Button>
                    <Button className='drop_btn' onCLick={() => {
                        setModal(state => !state);
                        setActive(false)
                    }}><Icon icon="icon-logout" color="#777E90"/>Log out</Button>

                </div>
            </Dropdown>
            <Modal active={modal} onClose={() => setModal(false)} className="modal">
                <div className='logout_icon'><Icon icon="icon-logout" color="#45B26B"/></div>
                <Title className="title">Oh no! You’re leaving...<br/>Are you sure?</Title>
                <div className='btn_row'>
                    <Button className='cancel' onCLick={() => setModal(false)}>{t("Cancel")?? "Cancel"}</Button>
