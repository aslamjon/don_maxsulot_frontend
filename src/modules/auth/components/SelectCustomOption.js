...
const CustomOption = ({
                    ...
                    <div className="content" onClick={() => isNil(res) && selectHandling(other)}>
                        <Flex justify={"space-between"} className="test">
                            <Flex align={"center"}>
                                ...
                                <span className={"hide"}>{`${get(
                                    other,
                                    "countryName"
                                )} (${get(other, `countryName[${0}]`)}${upperCase(
    ...
    return (
        isEmpty(options)
            ? <div className="select__body__options__empty"> {(action.create && !isEmpty(searchValue)) ? "Press Enter or click create button" : "Result not found"} </div>
            : isArray(options) && options.map((val, index) => (
            !isMulti && render({...val, index})
        ))
    )
}
