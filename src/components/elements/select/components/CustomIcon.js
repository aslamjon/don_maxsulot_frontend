...
const CustomIcon = ({isMulti, selected, isClearAll, clearAll, clickHeader}) => {
    return (
        (isClearAll && selected.length)
            ? <div className={"select__header__clearAll"} onClick={clearAll}>Clear All</div>
            : (isMulti && !isEmpty(selected))
                ? <div
                    className="select__header__selectedNumber">{selected.length > 1 ? `+${selected.length}` : 1}</div>
