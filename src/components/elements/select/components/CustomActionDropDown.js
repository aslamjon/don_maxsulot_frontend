...
const DropDownStyle = styled.div`
  .drop-down-dots {
    ${({color = "rgba(69, 178, 107, 0.1)"}) => color && css`
      background: ${color};
      height: 100%;
      display: flex;
      align-items: center;
...
const CustomActionDropDownn = ({
    ...
    return (
        <DropDownStyle color={color}>
            <Dropdown
                isFixed={isFixed}
                getXandY={(e) => console.log(e)}
                button={
                <div
                    onClick={(e) => ""}
                    className="drop-down-dots">
                    <Icon
                        icon="icon-more-dots"
                ...
                {action.delete &&
                    <div className="dropdown__option" onClick={() => clickDelete({optionsIndex, selectedIndex})}>
                        <Icon icon="icon-recycle" color="#EF466F" mainClassName='notClose'/> Delete
                    </div>}

                {action.edit &&
                    <div className="dropdown__option" onClick={() => clickRename({optionsIndex, selectedIndex})}>
                ...
                {get(action, "items",[]).map((Item, index) => <Item key={index+Math.floor(Math.random() * 999999)} options={options} optionsIndex={optionsIndex} />)}
            </Dropdown>
