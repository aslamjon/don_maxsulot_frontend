...
const CountdownTimer = ({defaultTime=60,resend = () => {},...rest}) => {
    ...
    return (
        <StyledCountdownTimer {...rest}>
            ...
            >
                {({ remainingTime }) => <span style={{ color: "#353945", fontSize: "14px", fontWeight: "600" }}>{remainingTime}</span>}
            </CountdownCircleTimer>
            <span className={'timer__text'}> Send code again</span>
            </>:<Button outline_success="1" className="SendAgainButton" center="1" onClick={restart} height="50"><img src={reloadImg} alt="reload" className="left-in-button" /> Send again</Button>}
