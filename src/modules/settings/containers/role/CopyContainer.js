    ...
    <div className="createContainer">
      ...
      <Box gray className={'mb-24'}>
        <FormBox title={"COPY ROLE"}>
          <FormDemo
            formRequest={create}
            footer={<Flex justify={'flex-end'} className={'buttons mt-30'}>
              <Button className={'cancelBtn'} onCLick={() =>  history.push('/setting/auth/roles')} outlineDanger>CANCEL</Button>
              <Button type={'submit'} className="addBtn" success>COPY ROLE</Button>
            </Flex>}
          >
            <Row className="form-content-body">
              ...
              <Col xs={6}>
                <Field type={'custom-select'}
                       ...
                       name={"roleType"} label={"Role type"} params={{required: true}}/>
              </Col>
              <Col xs={12} className={'mt-20'}>
                <Field type={'textarea'} defaultValue={get(data,"description")} name={"description"} label={"Description"}
        ...
        <Row>
          <Col xs={12}>
            {
              get(data,'modules',[]).map((module,index)=><FormDemo getValueFromField={getValueFromField} mainClassName={'main-form-table mb-24'}>
                <div className="main-form-table-head">
                  <Field type={'checkbox'} name={"module"} />{get(module,'name')}
                </div>
