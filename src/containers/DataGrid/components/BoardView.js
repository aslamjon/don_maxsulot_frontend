...
const BoardView = ({
    ...
    useEffect(() => {
        if (statuses.length !== get(headerData, "statuses", []).length) {
            ...
            setStatusWithPageNumberList(get(headerData, "statuses", []).map(item => (
                {
                    statusId: item.id,
                    page: 0
                }
    ...
    useEffect(() => {
        if (!isEmpty(get(idList, 'result.data.genericResult', []))
            && isEmpty(idListData)
        ) {
            setIdListDataWithLoop(get(idList, 'result.data.genericResult', []));
        } else if (!isEmpty(get(idList, 'result.data.genericResult', []))
            && get(idList, 'result.data.statusId', '')) {
            ...
            if (!isEmpty(nextIdList)) {
                statusWithPageNumberList[findIndex(statusWithPageNumberList, function (o) {
    ...
    const getCountByStatusId = (statusId) => {
        return get(idListData.find(item => item.statusId == statusId), "count", 0);
    }
                        ...
                        {({scrollableNodeRef, contentNodeRef}) => (
                            <div ref={scrollableNodeRef}>
                                <div className="horizontalSimpleBarBody" ref={contentNodeRef}>
                                    {isArray(statuses) && statuses.map((val, ind) => (
                                                ...
                                                {({scrollableNodeRef, contentNodeRef}) => (
                                                    <div
                                                        ...
                                                        onScroll={({target}) => {
                                                            ...
                                                            if ((bottom - 200) < target.scrollTop && (bottom - 50) > target.scrollTop && isFetched) bottomScrollHandling(val);
                                                        }}>
