    ...
    const handleChange = (value) => {
        if ((value != initialValue)) {
            ...
            let requestData = {
                id: rowId,
                // viewId,
                attributes: {[id]: (value === "CLEAR") ? null : value}, cb: {
                    success: () => {
                        setLoading(false);
    ...
    const CustomOption = ({
        ...
        return (
            ...
            isEmpty(options)
                ...
                : isArray(options) && options.map(({value, label, ...other}, index) => (isEmpty(selected) ? value !== "CLEAR" : true) && (
                    <div className={"select__body__options__option"} key={value}
                         onClick={() => selectHandling({value, label, ...other})}>
                        <Icon
                            mainClassName={"Icon"}
                            icon={value === "CLEAR" ? "icon-exit" : "icon-flag2"} color={(value === "URGENT")
                            ? "rgb(245,0,0)"
                            : (value === "HIGH")
                                ? "rgb(255,204,0)"
                                : (value === "NORMAL")
