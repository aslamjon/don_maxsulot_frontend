...
function* changeModuleOrDepartmentOrPageOrder(action) {
    ...
    try {
        ...
        yield put({ type: Actions.CHANGE_MODULE_OR_DEPARTMENT_OR_PAGE_ORDER.SUCCESS, payload: { data } });
        // yield call(cb.success,{...data});
    } catch (e) {
        yield put({ type: Actions.CHANGE_MODULE_OR_DEPARTMENT_OR_PAGE_ORDER.FAILURE });
...
function* updateModuleOrDepartmentOrPageTitle(action) {
    ...
    try {
        ...
        yield put({ type: Actions.UPDATE_MODULE_OR_DEPARTMENT_OR_PAGE_TITLE.SUCCESS, payload: { data } });
        yield put({ type: Actions.SET_LOADER.SUCCESS });
        yield call(cb.success, { ...data });
    } catch (e) {
        yield put({ type: Actions.UPDATE_MODULE_OR_DEPARTMENT_OR_PAGE_TITLE.FAILURE });
        yield put({ type: Actions.SET_LOADER.SUCCESS });
        const isField = false;
...
function* fileUpload(action) {
    ...
    try {
        const { data: { data } } = yield call(ApiService.FileUpload, attributes, url);
