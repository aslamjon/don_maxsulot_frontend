                    ...
                    <ComponentBodyStyle {...rest}>
                        <Modal active={open} onClose={() => !disabledOnClose && closeModal()}>
                            ...
                            {!isNil(get(item, 'entityName', null) || get(item, 'result', null)) ? (get(item, 'isFetched', false) ?
                                    <ModalBodyItem item={get(item, 'result.data', {})} btnText={'Save'}
                                                   addOrEdit={(id, data) => addOrEdit(id, data)} cancel={closeModal}
                                                   remove={remove}
                                                   hasModal={hasModal}
                                                   id={get(removeConfirm, 'id', null)}
                                                   confirmText={get(removeConfirm, 'name', '-')} {...dataForModal} /> : 'Loading ....') :
                                <ModalBodyItem item={get(item, 'result.data', {})}
                                               ...
                                               btnText={'Add'}
                                               hasModal={hasModal}
                                               confirmText={get(removeConfirm, 'name', '-')} {...dataForModal  } />}
                        </Modal>
                        <ComponentTable data={items} columns={columns} remove={setOpenModal} update={getOneRequest} hasModal={hasModal} redirect={redirect}/>
                    </ComponentBodyStyle>
                </Col>
            </Row>
        </Box>
