        ...
        <Row>
          <Col xs={12}>
            ...
            <ListTable columns={['Role name', 'Description', 'Action']}>
              {
                data && isArray(data) && data.map((item) => <tr>
                  ...
                  <td>
                    <Button onCLick={() => history.push(`/role/copy/${get(item, 'id')}`)} className={'action_btn mr-5'}><Icon
                      icon={'icon icon-copy'} color="#353945"/></Button>
                    <Button onCLick={() => history.push(`/role/edit/${get(item, 'id')}`)}
                            className={'action_btn mr-5'}><img src={edit2} alt="edit2"/></Button>
                    <Button onCLick={() => SetOpenModal(get(item, 'id'),"Yes, Delete")}  className={'action_btn mr-5'}><img
                      src={recycle}
                      alt="trash"/></Button>
                  </td>
                </tr>)
